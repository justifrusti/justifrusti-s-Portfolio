...
public class Environment : MonoBehaviour
{
    ...
    public static LivingEntity SenseFood(Coord coord, Animal self, System.Func<LivingEntity, LivingEntity, int> foodPreference)
    {
        ...
        for (int i = 0; i < foodSources.Count; i++)
        {
            ...
            if(EnvironmentUtility.TileIsVisible(coord.x, coord.y, targetCoord.x, targetCoord.y))
            {
    ...
    void Init()
    {
        ...
        for (int i = 0; i < initialPopulation.Length; i++)
        {
            ...
            if (initialPopulation[i].prefab is Animal)
            {
                Animal hunter = (Animal)initialPopulation[i].prefab;
        ...
        Coord[] viewOffsetArr = viewOffsets.ToArray();
        ...
        for (int y = 0; y < terrainData.size; y++)
        {
            for (int x = 0; x < terrainData.size; x++)
            {
                ...
                if(walkable[x, y])
                {
                    ...
                    {
                        int targetX = x + viewOffsetArr[i].x;
                        int targetY = y + viewOffsetArr[i].y;

                        if(targetX >= 0 && targetX < size && targetY >= 0 && targetY < size)
                        {
                            if(terrainData.shore[targetX, targetY])
                            {
                                if(EnvironmentUtility.TileIsVisible(x, y, targetX, targetY))
                                {
    ...
    void SpawnInitialPopulation()
    {
        ...
        foreach (var pop in initialPopulation)
        {
