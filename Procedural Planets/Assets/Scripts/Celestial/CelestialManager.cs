...
public class CelestialManager : MonoBehaviour
{
    ...
    [SerializeField] private bool procedurallyGeneratedSystem;
    [SerializeField][Range(0f, 1f)] private float systemScrub;
    [SerializeField] private CelestialBody systemStar;
    [SerializeField] private TextAsset systemBodyData;
    ...
    private void Awake()
    {
        ...
        if(procedurallyGeneratedSystem)
        {
            ReApplyProceduralTextures();
        }
    }
    ...
    private void ReApplyProceduralTextures()
    {
        Planet[] p = FindObjectsOfType<Planet>();

        foreach (Planet pl in p)
        {
            pl.GeneratePlanet();
        }
    }
    ...
    /// <summary>
    /// Simple procedure to load a solar system from a set of JSON files.
    /// </summary>
    public void LoadSystemData(TextAsset bodyData)
    {
        ...
        for (int i = 0; i < bodies.bodies.Length; i++)
        {
            ...
            GravityAttractor attractor = planetInstance.AddComponent<GravityAttractor>();
            attractor.gravitationalField = planetInstance.radius * 1.5f;
            attractor.showGizmos = true;
