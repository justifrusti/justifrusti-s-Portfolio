...
{
    [SerializeField] private BodyType bodyType = BodyType.Planet;
    ...
    [SerializeField] private float orbitalInclination;
    [SerializeField][Tooltip("Point of furthest distance")] private float apoapsis;
    [SerializeField][Tooltip("Point of closest approach")] private float periapsis;
    [SerializeField][Range(0f, 360f)][Tooltip("Angle from the body's ascending node to its periapsis")] private float argumentOfPeriapsis;
    [SerializeField][Range(0f, 180f)][Tooltip("Also known as obliquity")] private float axialTilt;
    [SerializeField] private float rotationPeriod;
    [SerializeField] private float orbitalProgress;
    [SerializeField] private float rotationProgress;

    [SerializeField] public int completeOrbits;

    [SerializeField] public float rawOrbits;

    public BodyType typeOfBody { get { return bodyType; } }
    public CelestialBody parentalBody { get { return parentBody; } } 
    ...
    public void InitializeBody(BodyData data, CelestialBody parentBody)
    {
        ...
        ApplyMaterials.Instance.ApplyMaterial(this);
    }

    /// <summary>
    /// Editor functionality to forcefully advance the orbit given a scrub value in the range 0 - 1
    /// </summary>
    ...
    {
        float orbitStep = (365 / orbitalPeriod) * GlobalTime.YearTick;
    ...
    /// <summary>
    /// Computes the transform along an orbital path given a progress value in the range 0 - 1
    /// </summary>
    public void CalculateOrbit(float orbitalProgress, float rotationProgress)
    {
        ...
        float siderealDay = rotationProgress * 360f * (bodyType == BodyType.Satellite ? 0f : 1f);
        ...
        transform.position = orbitPoint + parentalBody.transform.position;
    }
