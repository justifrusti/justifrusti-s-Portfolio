...
{
    [SerializeField] private bool drawOrbitLine = true;
    [SerializeField] private bool drawMarker = true;
    [SerializeField] private bool cacheOrbitPath = true;
    [SerializeField][MinMax()] private Vector2 minMaxLineWidth = new Vector2(8f, 20f);
    [SerializeField][Range(3, 5000)] private int lineResolution = 100;
    ...
    private void InitializeMarker()
    {
        ...
        switch(celestialBody.typeOfBody)
        {
            case BodyType.Star:
    ...
    private void UpdateOrbitDisplay()
    {
        ...
        float orbitalRadius = Vector3.Distance(celestialBody.transform.position, celestialBody.parentalBody.transform.position);
        float dstToOrbit = DistanceToOrbitalPlane(observerPoint, celestialBody.parentalBody.transform.position, orbitalRadius);
        ...
        if (drawOrbitLine)
        {
            ...
            float widthMultiplier = Mathf.Min(dstToOrbit * (minMaxLineWidth.x / 1000f), minMaxLineWidth.y);
